...
One full language (German, `de`) shipped 2026-09-20 — see §9's own entry for what it covered and the
real live-switch bug it surfaced and fixed along the way. Still open here: a community-translation
*workflow* (this was one hand-built language, not a contribution pipeline) and any other views found
still unlocalized. Spec: `spec/10-localization.md`. Pairs naturally with PA-5, which would catch
broken keys.
...
| **Mode change / "SSB as PKT" silently reverts on rigctld** — user-reported live bug, 2026-09-19 | **CLOSED 2026-09-19: root cause was the user's own rig stuck in DATA-U, not a Scanline Studio bug.** User confirmed after manually cycling the rig's mode from the front panel that both raw `rigctl` commands and current `master` work correctly, including "SSB as PKT" unchecking converting DATA-U back to USB as designed. Two real app-side fixes were still shipped along the way and remain worth keeping (confirmed as genuine defects by `yoniq-auditor` + `yoniq-principal`, just not the cause of this specific report): (1) `RadioStatusViewModel._ssbAsPktOperatorOwned` — a poll-driven `SyncSsbAsPktFromPolledMode` (added in `37ea13d`) was unconditionally re-deriving the "SSB as PKT" checkbox from the rig's live mode every ~250ms, fighting an operator's own click; now stops once the operator has interacted, mirroring the existing `_ssbAsPktCameFromRig` one-shot convention. (2) `SetModeSafeAsync`'s catch split three ways (`ArgumentOutOfRangeException`/`RadioProtocolException`/generic) so a genuine rig rejection surfaces the real `RPRT -<n>` text instead of a generic "No radio connected." A separate bandwidth-fallback fix (mirroring `ApplyPresetAsync`'s existing default-passband-landmine avoidance into `SetModeSafeAsync`) also shipped but was likely irrelevant to the actual symptom. A third suspected mechanism (a non-atomic bandwidth read-modify-write in `RigctldClientProtocol.SetBandwidthAsync`) was investigated and downgraded to a real-but-low-probability hygiene issue (Hamlib caches the just-set mode for 500ms by default) — not fixed, not blocking, deferred (would touch 4 backends). 4 new regression tests added for the two shipped fixes; `dotnet test` green across `ScanlineStudio.UI.Tests` (1740). **Still open, separate from this bug**: the original "loading a saved 7.171 Data-R preset doesn't switch to Data-L" report — never reproduced or explained by any of the above; may just be Scanline's `DataR` label vs. the rig's own "Data-L" naming for the same underlying mode, needs the user to retest specifically. |
| **TX editor: right-clicking the background image has no "Save Template" option** — user-reported, 2026-09-19 | **Fixed.** `SaveTemplateWithAutoNameAsync` (auto-fills the next free "tmp{N}" name and saves) already existed and was already wired to the canvas's own empty-area `ContextMenu` (`Canvas.ContextMenu`, `TxImageEditorPaneView.axaml`) — but that menu is unreachable the instant a background/backdrop image exists, since that element's `Border` stays unconditionally hit-testable (a deliberate 2026-09-15 fix so right-click reaches the backdrop's own menu, not the canvas's) — so a right-click always resolved to the image element's own menu instead, which had no equivalent item. Added the same `MenuItem` (same `Click="OnSaveTemplateContextMenuClick"` handler, no new logic) to the image/background element's own `ContextMenu`, always enabled there (reaching that menu at all means an element already exists to save). Also, user-requested: auto-naming now starts at `tmp0`, not `tmp1`. 2 existing tests renamed/updated for the new numbering; `dotnet test` green across `ScanlineStudio.UI.Tests` (1740); `docs/help/index.html`'s "Save and recall templates" step updated for both the new reachability and the 0-based numbering, `node docs/help/check-help.mjs` passes. |
| **Keep the TX editor open across Apply/Transmit; remove the Ready Rack pileup feature** — direct user request, 2026-09-19 | **Shipped 2026-09-19.** Plain Apply and Apply & Transmit no longer close/dispose the TX editor — `TxControlsPaneViewModel.OnEditorApplied` keeps `_currentEditor`/`IsEditorOpen` untouched (dropping only the old close/dispose/`EditorClosed` calls, and directly re-notifying `CanChangeSourceOrMode`/`CanLoadBackground`/`QuickSelectModeCommand` since `OnIsEditorOpenChanged` no longer fires for free). "New Template" (`CancelCommand`, already relabeled from "Cancel" in an earlier session) is now the ONLY action that closes/resets the editor; `OnEditorCancelled` always reopens a fresh blank one afterward, unconditionally. Removed `EditCurrentImageAsync`/`CanEditCurrentImage`/`EditCurrentImageCommand` and its button as dead code — there is no remaining way to reach it once Apply never closes the editor. The Ready Rack "pileup"/direct-fire feature (Ctrl+1..9: load a template onto the current photo, bake, transmit, auto-reopen) is removed entirely, by direct request ("we do not queue pictures") — `ReadyRackViewModel.DirectFireSlotCommand`/`TemplateDirectFireRequested`, `TxImageEditorPaneViewModel.OnReadyRackDirectFireRequested`/`DirectFireRequested`/`_pendingDirectFireTemplateId`, `TxControlsPaneViewModel.OnEditorDirectFire`, the Ctrl+1..9 keybinding block in `TxImageEditorPaneView.axaml.cs`, and ~20 tests are all gone; the plain no-modifier 1-9 recall shortcut is untouched. TX history's Resend was restructured so it never disturbs the live editor ("leave the current TX editor image alone") — extracted a shared `TransmitCoreAsync(image, mode, sourceFileName, refreshMacros)` that both ordinary Transmit and `ResendSentFrameAsync` call; Resend passes only the historical entry's own fields and never reads/writes `SelectedMode`/`_editState`/`_loadedImage` at all, so it cannot diverge the editor's own read-only `_targetMode` the way an earlier "suppress-flag" draft design would have (same failure class as the historical 86d07d5 stale-mode-transmit-crash, but quieter — several SSTV modes share pixel dimensions and would have passed the dimension backstop silently). `refreshMacros: false` on Resend is load-bearing, not an optimization: `RefreshTransmitImageIfMacrosChangedAsync`'s own "no change" return path is NOT guarded by `SelectedMode == mode`, so calling it unconditionally would have silently substituted the live editor's own canvas for the historical frame. A same-day follow-up fix: `OnModeDetected`'s "auto-follow RX mode" feature used to refuse whenever `IsEditorOpen`, which — now that Apply never closes the editor — silently disabled auto-follow for the rest of every session after the first Apply. Checked against legacy (`TMmsstv::TrackTxMode`, `Main.cpp:4907-4914`): no window-open check exists there at all (legacy has no separate TX editor window), only Fixed-TX-Mode/TX-active/width-match — removed the `IsEditorOpen` check to match, relying on the already-audited `ReplaceEditorForModeSwitch` mechanism (shipped earlier the same day) to make an RX-driven mid-edit mode switch safe. 3 `yoniq-auditor` plan-review rounds + 1 `yoniq-principal` round (before implementation, on the Resend design) + 1 `yoniq-auditor` code-review round (after) — the code-review round found one real bug in the `OnModeDetected` fix: it compared width against `_loadedImage` (null until the first Apply), so a real, unapplied photo sitting in an open editor got no width protection at all; fixed by comparing against `SelectedMode`'s own width instead (the real analogue of legacy's always-live `pBitmapTX->Width`), gated on the same "safe to disrupt" predicate `CanChangeSourceOrMode` already uses. Principal review of the code-review was explicitly not needed (auditor's own answer: no blocker, no round-to-round contradiction, UI/view-model state not decode-path DSP, and the legacy claim was independently re-verified, not asserted). **Known, accepted workflow change** (not a bug): Browse/Stock/File > Open/Copy-to-TX now stay disabled after the first Apply until "New Template" is clicked, persistently, not just mid-edit. `dotnet test` green across `ScanlineStudio.UI.Tests` (1712, +regression test for the width-guard bug). |

| **Make TX progress more obvious** — direct user request, 2026-09-19 | **Shipped.** The small, easily-missed transmit-progress bar (stacked ProgressBar + percent/remaining text) is removed from the narrow TX sidebar (`TxControlsPaneView.axaml`) and replaced with a bigger one in the TX image editor's own wide context bar — the header row already showing "TX IMAGE EDITOR \| OUTGOING FRAME 640×496 · PD120 · ...", visible only while transmitting. `TxImageEditorPaneViewModel` has no transmit state of its own (a separate, shorter-lived instance recreated across Apply/mode-switch/New Template), so this follows the same "no duplicated flag, parent pushes on every toggle point" pattern already established for `_canTransmitNow`/`CanApplyAndTransmit`: a new `_transmitStatusNow` snapshot delegate (`(bool IsTransmitting, double? Progress, string ProgressText)`), threaded through both real construction call sites, and `NotifyTransmitProgressChanged()` called at all 3 places the parent's `IsTransmitting`/`TransmitProgress` actually change (`TransmitCoreAsync`'s start and `finally`, `OnTransmitProgressChanged`'s per-tick update). One lighter-tier `yoniq-auditor` code-review pass (not a full plan-review — pure UI/display work, no legacy counterpart, no decode-path/concurrency risk): verdict EQUIVALENT, no defects in delegate threading, push-site completeness, or stale-editor risk (structurally impossible — every `_currentEditor` swap is synchronous with its `EditorOpened`/`EditorClosed` event, and the replacement editor's getters read the live closure). One cosmetic nit fixed (a `MinWidth="80"` on the ProgressBar so it can't visually compress to nothing at a narrow width), which itself required updating `AtomsFontScaleTests`'s pinned literal count for `TxImageEditorPaneView.axaml` (109→110) — a fixed space-reservation literal, same class as that file's own pre-existing `MinWidth="180"`/`"120"` literals, not a font-size-coupled site. New test (`TxControlsTransmitProgressTests.TransmitProgress_PushesToTheOpenEditorsHeaderBar_AndRaisesPropertyChanged`) drives the real production open path and asserts the `PropertyChanged` notifications themselves, not just resulting values. `docs/help/index.html`'s existing progress sentence was already location-agnostic, no update needed; `node docs/help/check-help.mjs` passes. `dotnet test` green across `ScanlineStudio.UI.Tests` (1713). |

| **"Transmitting" label on header + status bar; remove TX KEYED chip** — direct user request, 2026-09-19 | **Shipped.** The Transceiver card's Receiving/Decoding toggle and the bottom status bar's RX chip now both read "Transmitting" for the whole window this app is keying the rig (a real Transmit or a Tune tone), reverting to Receiving/RX Muted/Decoding once it ends. Removed the bottom status bar's separate TX KEYED chip (`RadioStatus.IsKeyed`, rig-CAT-PTT-readback-dependent, silently dark forever on VOX/DTR-keyed rigs) as redundant now that the RX chip covers it more reliably. Backed by a new `RadioStatusViewModel.IsTransmittingDisplay`, itself an OR of the pre-existing event-driven `IsCapturePausedForTx` (near-instant, but — per a same-day `yoniq-auditor` lighter-tier code-review finding — only fires when RX capture was ACTUALLY running when the transmit/tune started) and a new polled `IsSstvTransmitting` (mirrors `ISstvSessionService.IsTransmitting`, unconditional on prior RX state, polled on the existing 250ms `_rxAudioLevelTimer` tick) — closes a real gap the first draft had: starting a Transmit while "Receiving" was already toggled off used to leave both labels silently stuck on "RX Muted" for the entire transmission. `RadioStatus.IsKeyed`/`VfoKickerDisplay` (VFO card's "RX"/"TX" kicker) are unchanged and out of scope — not part of what the user asked to change. 2 new tests (`WhileTransmitting_...SayTransmitting_EvenOverADecodeInProgress`, `WhileRxIsHalted_StartingATransmit_StillSaysTransmitting`), both asserting the actual `PropertyChanged` notifications, not just resulting values. `docs/help/index.html` updated (new "Transmitting" state documented, TX KEYED definition removed), `node docs/help/check-help.mjs` passes. `dotnet test` green across `ScanlineStudio.UI.Tests` (1715). **Known, accepted cosmetic gap** (auditor nit, explicitly not worth another review round): the status-bar chip's own tint/LED color classes don't yet reflect the new Transmitting state — it can show green/amber styling next to "Transmitting" text during a transmit, since only the header toggle's pre-existing Opacity dim (a separate, already-shipped 2026-08-18 fix) visually de-emphasizes TX. |

| **Gallery right-click context menu** — direct user request, 2026-09-19 | **Shipped.** Right-clicking a Gallery thumbnail now selects it and shows a menu with the same 6 actions as the Selected-frame panel below: Open in log, Export, Send to TX, Flag/Unflag, Add note..., Delete. The first 4 reuse the existing `SelectedEntry`-scoped commands unchanged; right-click selects the entry first (`ContextRequested`, fires before the popup opens), matching this pane's existing single-selection model. Two new commands cover the 2 previously property-only actions: `ToggleSelectedEntryFlagCommand` (synchronous, no race possible — verified no `await` sits between the `SelectedEntry` read and the write) and `AddNoteToSelectedEntryAsync`, which opens a newly generalized `TextPromptWindowViewModel` (was hard-wired to `IConfigurationPresetStore` validation; now takes an optional `validate` delegate, defaulting to always-valid — the one existing Configurations caller passes a wrapper preserving its old behavior exactly). `yoniq-auditor` code-review found and this fixed one real blocker before shipping: the "is this entry still selected" re-check after the dialog closes used `ReferenceEquals`, which reads false even for the SAME logical entry once `RefreshAsync` rebuilds it (a fresh `RxHistoryEntryViewModel` instance per refresh, by design) — left the visible note field showing stale text after a background refresh landed mid-dialog, risking the next keystroke re-persisting the old text over the new one. Fixed with an Id-based comparison; also fixed a related, auditor-found gap where a pending 600ms debounced write from the details panel's own TextBox could fire after the dialog's write and silently clobber it — now cancelled before the dialog's own write commits. Both fixes are mutation-verified (temporarily reverted each, confirmed the exact regression, then confirmed the new test catches it). 6 new tests total. `docs/help/index.html`'s Gallery topic documents the new right-click menu; `node docs/help/check-help.mjs` passes. `dotnet test` green across `ScanlineStudio.UI.Tests` (1721). **Known, accepted limitation** (auditor nit, not fixed): the menu is mouse-only — a keyboard-invoked context menu (Apps key/Shift+F10) won't reach it, since `ContextMenu` is attached to a Border nested inside the ListBox's item template rather than the item container itself. |

| **Output card meters as numbers; blank-background card cleanup; Sent-frames right-click; template save/load checkpoint** — direct user request, 2026-09-19, 4-part batch | **Shipped.** (1) Power/Alc in the Output card now show only their existing numeric rows (`Text="{Binding LivePowerPercent, StringFormat={}{0:0}%}"` etc.) — the `IndustryMeter` fill-bar Grids and the "POWER / ALC METER" caption are removed (`TxControlsPaneView.axaml`), confirmed no watts telemetry exists anywhere in the radio layer so "%" (the only real available unit) stays, not literal watts. (2) The Background card's filename/preview now hide behind a new `ShowBackgroundPreview` property (`SelectedFileName is not null && editor.HasRealBackground`) instead of showing a stale "Blank" name/gray preview left over from a blank-canvas Apply. (3) The Sent-frames history's per-row Resend/Save buttons (too small to hit reliably) are replaced with a right-click `ContextMenu` reusing the same existing code-behind `Click` handlers unchanged (a `MenuItem`'s `DataContext` inherits through `ContextMenu`/`Border` the same way a `Button`'s did). (4) Template save/load "unsaved edits" checkpoint: a successful `SaveTemplateAsync` now resets the dirty baseline exactly like a successful load already did, so the header "UNSAVED EDITS" chip and "New Template"'s discard-confirm both clear on save and only re-arm on a REAL edit afterward — new public `IsDirtySinceLastCheckpoint` (`_editVersion != _editVersionAtLastCheckpoint || _carriedOverDirtySinceCheckpoint`) now backs both surfaces (and the Ready Rack "Loaded • edited" badge, its original sole consumer), while the older `HasUnsavedEdits` (`_undoStack.Count > 0 || _carriedOverUnsavedEdits`) is deliberately left unchanged for its own 3 remaining consumers. A `yoniq-auditor` code-review round on item 4 found 2 real blockers, both fixed and mutation-verified before a clean go: **Blocker 1**, `_carriedOverUnsavedEdits` is `readonly` and was OR'd directly into the new property, so a mode-switched editor's checkpoint could never clear — fixed with a separate, independently-clearable `_carriedOverDirtySinceCheckpoint` field, seeded the same way but cleared at both checkpoint sites, leaving `HasUnsavedEdits`/`_carriedOverUnsavedEdits` untouched. **Blocker 2**, comparing `_undoStack.Count` against a saved baseline silently read "clean" once the stack pinned at `MaxUndoDepth` (50) and a checkpoint happened to land there too (every further push nets back to Count=50) — a real, silent-data-loss path via an unconfirmed "New Template" discard — fixed with a new unbounded `_editVersion` counter that mirrors the stack's own push/Undo-pop/Redo-push transitions but is never decremented by the 50-cap's own eviction. A follow-up `yoniq-auditor` round (traced the eviction-then-undo edge case by hand, re-enumerated every `_undoStack`/`_redoStack` mutation site) confirmed both fixes hold with no remaining path to either failure mode, gave a clean go, and found one further cosmetic risk (the load-side checkpoint reset changed `IsDirtySinceLastCheckpoint` without re-notifying, so the chip/badge could briefly show stale-dirty across the load's own remaining awaits) — fixed the same day by moving the notify calls into `LoadTemplateIntoLiveEditor` itself, right after the checkpoint reset. 5 new regression tests (3 for the base checkpoint behavior, 2 targeting the two blockers specifically), all mutation-verified. `dotnet test` green across `ScanlineStudio.UI.Tests` (1726); `docs/help/index.html`'s Sent-frames step updated for the right-click model, `node docs/help/check-help.mjs` passes. **Known, accepted gaps** (auditor nits, not fixed): `StringFormat={}{0:0}%` on a null Power/Alc value hasn't been live-checked against a real/disconnected rig for a bare "PWR  %" render; a pre-existing "undo once then make a different edit" false-clean hole in the dirty check is unchanged by this fix (the old Count-based version had the identical hole, just wider). |

| **Rename templates in place; drop the action-strip name field** — direct user request, 2026-09-19 | **Shipped** (`e3fd1d6`, help `d7b1682`). Both the TEMPLATES rack card and the TEMPLATE LIBRARY card had an always-visible name `TextBox` above their Load/Pin(Unpin)/Export/Delete row; the right-click Rename item worked by focusing that box, so deleting it outright would have broken rename. Replaced with in-place editing: a per-row `TemplateListRowViewModel.IsRenaming` flag swaps the row's name `TextBlock` for a `TextBox` (shared by a pinned slot and its Library row, since they are the same instance); the context-menu Rename item sets it and focuses the box via `ItemsControl.ContainerFromIndex` + a class-tagged descendant search (`FocusInlineRenameBox`), because an `x:Name` inside a `DataTemplate` is not a field on the view. Enter commits, Esc reverts, blur commits (guarded so the blur caused by the box collapsing cannot double-commit). The rack's box reassigns `DataContext="{Binding Template}"` so one `OnInlineRenameKeyDown`/`OnInlineRenameLostFocus` pair serves all three row templates. `RenameTemplatePlaceholder` locale key removed as orphaned. Live-verified on both cards. **Known, accepted gap**: two rows can be left in rename mode at once if a second Rename is invoked before the first commits — cosmetic, each still commits independently. |
| **Plain-English pass over the help guide** — direct user request, 2026-09-19 | **Shipped** (`9d1238b`). Wording only across `docs/help/index.html`: em-dash asides 74→3, semicolon-joined clauses 57→1, "its own/itself" filler 15→1, justifying clauses cut, emphasis caps replaced. Every topic ID, link, `data-keywords`, `data-category`, and `<code>` value verified byte-identical before/after; `check-help.mjs` passes. Heaviest in `image-editor` and `templates-overlays`. |
| **R7 — Per-language help guide lookup with English fallback** — filed 2026-09-19, planned and shipped same day | **Shipped, infra only — no translated content authored (that is R2's job, still open).** English guide stays at `docs/help/index.html` (also the fallback). A translation lives at `docs/help/<languageCode>/index.html`, `<languageCode>` being `CultureInfo.TwoLetterISOLanguageName` — the same code `JsonLocalizationService` already keys `assets/locale/<languageCode>.json` on. New `HelpGuidePathResolver.Resolve` (`src/ScanlineStudio.UI/Services/HelpGuidePathResolver.cs`) is a small pure function `MainViewModel.UserGuidePath` now calls with `_localization.CurrentCulture.TwoLetterISOLanguageName` on every access — re-evaluated per click, so a mid-session language switch needs no `CultureChanged` subscription. `ScanlineStudio.Host.csproj`'s help-guide `<None>` items became one recursive glob (`docs\help\**\*.html;**\*.css;**\*.js`) so any `<languageCode>/index.html` copies to the build output automatically, no `.csproj` edit needed per language. `docs/help/check-help.mjs` now discovers and structurally validates every `docs/help/<dir>/index.html`, and fails if a translated guide's topic-ID set doesn't exactly match English's (drift-proofing). User decision: the 2 runtime chrome strings `help.js` generates ("No help topics found for…", "unversioned") also got localized, not left English-only — moved into a per-`index.html` inline `window.ScanlineHelpStrings` block (fetch-free, since the guide is a bare `file://` page with no server) that `check-help.mjs` also validates is present and non-empty. Verified via 2 throwaway fixture guides (one with a topic deleted, one missing the strings block) confirming `check-help.mjs` fails correctly, then removed; 3 new `HelpGuidePathResolverTests` cases pass; `dotnet test tests/ScanlineStudio.UI.Tests` green (1729); `dotnet build ScanlineStudio.sln` clean; `node docs/help/check-help.mjs` passes against the real English-only tree. Plan: `~/.claude/plans/reflective-whistling-shamir.md`. |

| **R2 — German translation (partial: content shipped, live-switch mechanism was broken and is now fixed)** — direct user request, 2026-09-19/20 | **Shipped.** `assets/locale/de.json` — full 1,058-string German UI translation, produced by 9 parallel Fable-model agents against one fixed terminology glossary (researched from the DARC-hosted German MMSSTV manual and Yaesu/Kenwood/Icom German manuals — real ham-radio usage, not literal translation; e.g. PTT/CAT/SWR/VOX stay English, "Transceiver" not "Funkgerät"/"Rig"). `docs/help/de/index.html` — full 31-topic German help guide via R7's own mechanism, 6 parallel chunks + a translated shell, passes `check-help.mjs`. `assets/locale/locales.json` gained the `de` entry. Cross-chunk terminology conflicts found and fixed (BPF filter labels, QSO-data bar name, background/backdrop wording, status-chip parallelism) before merge. **Real bug found during live verification, not a translation-content bug:** switching the interface language did not actually re-render most of the app. Root cause (found via a 2-round `yoniq-auditor` investigation + a `yoniq-principal` confirmation): `TranslateExtension.ProvideValue` (the `{loc:Translate}` markup extension backing ~900 XAML bindings app-wide) handed a freshly-built `TranslateBindingSource` straight to `Binding.Source` with nothing else referencing it — nothing roots a bare `Binding.Source`, so almost every source was garbage-collected before ever being read (empirically confirmed: 236 subscribers on `ILocalizationService.CultureChanged` right after opening Options, only 1 left by the time a switch fired), permanently freezing each bound control in whichever language was active when it was first built. Explains both reported symptoms: Options' own tab headers/labels not updating on Apply, and repeated switching leaving the Transmit editor (freshly rebuilt on every mode switch/New Template, 354 more `{loc:Translate}` sites) stuck in whatever language was active at that rebuild. Fixed in `TranslateExtension.cs`: each source is now rooted in a `ConditionalWeakTable<object, List<TranslateBindingSource>>` keyed on its own XAML target control, alive exactly as long as that control is, released automatically when the control is collected — same no-leak intent the existing weak `CultureChanged` subscription already had, just with an actual root underneath it. Also hardened `JsonLocalizationService.CultureChanged`'s raise site (`RaiseCultureChanged()`) with a per-handler try/catch, so one throwing subscriber can no longer silently mute every subscriber registered after it — closes a real gap the investigation surfaced even though it wasn't the actual trigger this time. Separately, this same investigation also touched the **live-refresh gap in 10 VM-level computed properties** (RadioStatus/Gallery/Logbook/Transmit/Options etc. not re-deriving `_localization.GetString(...)`-backed values on a culture switch) — fixed with a `CultureChanged`-driven `OnPropertyChanged(string.Empty)` blanket refresh per VM, correctly tiered between DI-singleton panes (simple subscribe) and genuinely-recreated VMs (paired subscribe/Dispose-unsubscribe, `TxImageEditorPaneViewModel`/`OptionsWindowViewModel`), plus a few ambient stored properties needing explicit re-derivation (`RadioStatusViewModel.FrequencyDisplay`/`BandwidthDisplay`, `RxImagePaneViewModel.SenseLevelOptions`/`RxBpfPresetOptions`, `LogbookPaneViewModel.AvailableModes`/`EntryCountDisplay`) — a `FormMode` selection-preservation risk around the `AvailableModes` rebuild was closed defensively (capture/restore, not dependent on unverified Avalonia `ItemsSource`-replacement behavior) and proven with a real regression test. **Known, accepted residual gaps** (both auditor-confirmed non-blocking, stale text only, no wrong value): the Favourites editor's own mode picker (`RadioStatusViewModel.PresetModeChoices`) and already-sent-frame outcome labels in the TX history list — both bake translated text into per-row child view-model instances at construction, which the parent-level fix can't reach without touching a second class per item; deferred as a separate, smaller follow-up if ever needed. 2 full `yoniq-auditor` review rounds (plan + code) on the VM-refresh work, a dedicated `yoniq-auditor` root-cause investigation + `yoniq-principal` confirmation on the `{loc:Translate}` bug — principal's explicit go: "Nothing I verified blocks shipping." `dotnet test` green across `ScanlineStudio.UI.Tests` (1732), `ScanlineStudio.Core.Localization.Tests` (23), `ScanlineStudio.Application.Tests` (590), `ScanlineStudio.Host.Tests` (74), `ScanlineStudio.UI.FontTests` (43, including a real-window regression test for the Options-dialog bug and a fix for one unrelated pre-existing failure the investigation surfaced along the way). Live-verified against the real running app both before and after. **Still open, separate from this fix:** R2's community-translation workflow itself (this shipped one language, hand-built, not a contribution pipeline) and PA-5 (broken-key checking) are unaffected and remain as filed. |
