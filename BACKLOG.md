...
| **Mode change / "SSB as PKT" silently reverts on rigctld** — user-reported live bug, 2026-09-19, IN PROGRESS | **Two rounds so far; not yet confirmed fixed live.** Round 1 (bandwidth-fallback fix, superseded below as insufficient): `SetModeSafeAsync` sent a bare mode-set with no follow-up bandwidth, matching a landmine `RadioModeFamilies`'s own doc comment already described (Hamlib's `RIG_PASSBAND_NORMAL` resolving to a destructive width, e.g. 500 Hz) — mirrored `ApplyPresetAsync`'s existing fix into `SetModeSafeAsync`. User reported this did NOT fix it, and specifically that FM also reverted, which this fix cannot explain (FM's fallback, 15000 Hz, is unremarkable). **Round 2, after `yoniq-auditor` + `yoniq-principal` review** (user explicitly requested the principal step): auditor ruled out an initially-suspected cross-thread race (the poll callback IS fully `Dispatcher.UIThread.Post`-marshalled, confirmed) and found the real regression in commit `37ea13d` ("Derive SSB-as-PKT checkbox from polled radio mode") — `SyncSsbAsPktFromPolledMode` made the `SsbAsPkt` checkbox an unconditional per-poll (~250ms) mirror of the rig's mode, but `IsSidebandUsb`/`IsSidebandLsb` read that SAME checkbox at click-time to decide which mode token to send, so the poll and the operator's own click can disagree about what "SSB as PKT" currently means. Principal independently confirmed the git-history claim (pre-`37ea13d`, `SsbAsPkt` was never poll-written) but downgraded a second suspected mechanism (a non-atomic bandwidth read-modify-write in `RigctldClientProtocol.SetBandwidthAsync`) to a real-but-improbable hygiene issue, since Hamlib caches the just-set mode for 500ms by default — and pointed out **this still doesn't explain FM**, which the `SsbAsPkt` logic never touches at all. Fixed so far: (1) new `_ssbAsPktOperatorOwned` latch — once the operator interacts with the checkbox or the USB/LSB segments, `SyncSsbAsPktFromPolledMode` stops overwriting it, mirroring the existing one-shot `_ssbAsPktCameFromRig` convention; (2) `SetModeSafeAsync`'s catch split three ways (`ArgumentOutOfRangeException` → new `RadioStatus.Error.ModeNotSupported`, `RadioProtocolException` → new `RadioStatus.Error.ModeRejected` carrying the real `RPRT -<n>` text, everything else → the existing generic `NoRadioConnected`) so a genuine rig rejection is diagnosable instead of indistinguishable from a dropped CAT link; (3) `CanSetBandwidth` now captured before the first `await`, matching `ApplyPresetAsync`/`SetFrequencyAsync`'s own convention. 4 new tests added. `dotnet test` green across `ScanlineStudio.UI.Tests` (1740). **Explicitly NOT done**: the bandwidth atomic-mode-set fix (finding 2, downgraded to hygiene, deferred — touches 4 backends) and the FM revert, which has no confirmed mechanism yet — needs the user to reproduce with the app's own log (`Help → Open application log`, `Log.SetModeFailed`/`Log.SelectedRadioModeChanged` lines) to determine if the rig is rejecting `M FM 0` outright or something else is happening. Also still open: the original "loading a saved 7.171 Data-R preset doesn't switch to Data-L" report, unexplained by any finding so far. |
