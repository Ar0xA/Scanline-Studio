...
### R7. Per-language help guide lookup with English fallback

Filed 2026-09-19. The help guide is not keyed to the UI language: `MainViewModel.UserGuidePath`
(`MainViewModel.cs:65`) opens the fixed path `help/index.html`, and `docs/help/` holds one English
file (`<html lang="en">`). The UI string table goes through `loc:Translate` and can take a second
locale; the guide cannot. Nothing is needed while `assets/locale/locales.json` lists only `en`.
**Do when a second locale lands, alongside R2:** resolve `help/<lang>/index.html` from the active
locale, fall back to `help/index.html`, and make `docs/help/check-help.mjs` validate every
language's copy against the same topic-ID set so a translated guide cannot drift out of sync.
