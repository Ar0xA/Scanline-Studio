...
| **Mode change / "SSB as PKT" silently reverts on rigctld, sticking on a wrong (e.g. 500 Hz) bandwidth first** — user-reported live bug, 2026-09-19 | **Fixed.** `RadioStatusViewModel.SetModeSafeAsync` (the ONE call site both the mode ComboBox/segment buttons and `OnSsbAsPktChanged`'s mode-conversion funnel through via `SelectedRadioMode`) sent a bare mode-set with no follow-up bandwidth — exactly the landmine `RadioModeFamilies`'s own doc comment already described: a mode-set asks the rig for its own default passband (Hamlib's `RIG_PASSBAND_NORMAL`), which on some rigs resolves to a destructively narrow width (confirmed: 500 Hz), badly enough that the rig fell back to its prior mode on the next poll. `ApplyPresetAsync` already avoided this by sending `RadioModeFamilies.FallbackFor(mode)` (2400 Hz for the SSB family) right after the mode-set — mirrored into `SetModeSafeAsync` instead of inventing a second mechanism, gated on `CanSetBandwidth` for the same reason (flrig/OmniRig throw rather than no-op). Not yet confirmed against the user's own rigctld setup live; confirmed only by reasoning + new unit tests. 3 new tests added (`RadioStatusViewModelTests.cs`): manual switch to an SSB-family mode sends the fallback bandwidth in the right order, the SSB-as-PKT-driven conversion does too, and a mode with no family (Cw) sends none (so an operator's own deliberately narrow CW filter is never silently overwritten). `dotnet test` green across `ScanlineStudio.UI.Tests` (1736). **Open**: the user's separate report that loading a saved 7.171 Data-R preset "doesn't switch to Data-L" wasn't explained by this fix — `ApplyPresetAsync` already sent the correct fallback bandwidth before this change, so that symptom (if still present) has a different cause, possibly just Scanline's `DataR` label vs. the rig's own "Data-L" naming for the same underlying PKTLSB mode. Needs the user to retest and report back. |
