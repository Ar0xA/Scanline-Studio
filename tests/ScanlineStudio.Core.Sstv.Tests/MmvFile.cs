...
///
/// Kept in the test project rather than <c>ScanlineStudio.Core.Sstv</c>: this format has no production
/// consumer (Scanline Studio doesn't read/write `.mmv` files as a feature; it exists purely so a developer
/// with a real legacy install can capture golden-vector fixtures by hand), so promoting it would
/// add a public surface nothing else needs. Don't "helpfully" move this into `ScanlineStudio.Core.Sstv`
