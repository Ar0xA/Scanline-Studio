...
- [[06-sstv-dsp]]: encode/decode round-trip passing for the core mode set, against `FakeAudioEngine` and real audio; sample-clock calibration (slant correction) and FSK/CW station ID implemented (corrected into scope after review — these are correctness/regulatory requirements, not polish, see [[06-sstv-dsp]]).
  - Mode-by-mode sequencing, each verified individually before moving to the next (explicit user instruction, not a shortcut): Martin M1/M2, Scottie S1/S2/DX, and Robot 36 are done — channel order/timing/tone frequencies read from the real TX line-generator functions in `Main.cpp` (`LineMRT`, `LineSCT`, `LineR36`), each cross-checked against `CSSTVSET::GetTiming`. Robot 36 needed a real architecture change (per-family `IScanlineEncoder`/`IScanlineDecoder` strategies via `ScanlineCodecFactory`, since its YCbCr-with-alternating-chroma scheme doesn't fit Martin/Scottie's shape) — that architecture is now in place for future families to plug into. Remaining, each still needing its own from-source tracing: Robot 72 (confirmed NOT the same family as Robot 36 despite the name — it's grouped with the MR/ML family in `Main.cpp`'s case statements), the PD-series (YCbCr, two-lines-per-transmission-line), Pasokon P3/P5/P7, the MR/ML/MP/MN/MC/R24/RM8/RM12 "half-scan-chroma" family, AVT.
