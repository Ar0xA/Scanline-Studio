...
- [[06-sstv-dsp]]: encode/decode round-trip passing for the core mode set, against `FakeAudioEngine` and real audio; sample-clock calibration (slant correction) and FSK/CW station ID implemented (corrected into scope after review — these are correctness/regulatory requirements, not polish, see [[06-sstv-dsp]]).
  - Mode-by-mode sequencing, each verified individually before moving to the next (explicit user instruction, not a shortcut). **31 of 43 modes done** (`SstvModeRegistry`, all cross-checked against `CSSTVSET::GetTiming`, all read from the actual TX line-generator functions in `Main.cpp` per CLAUDE.md's TX/RX-split and no-assumptions rules — several near-misses caught this way: Scottie's real sync position, Robot 72 vs. Robot 36, MP vs. MR): Martin M1/M2, Scottie S1/S2/DX, Robot 36/72, AVT, MR73–175, ML180–320, MP73–175, the whole PD-series, Pasokon P3/P5/P7. Four scanline-codec families exist now (`ScanlineCodecFactory`): `RgbSequential`, `YCbCrRobot` (alternating chroma), `YCbCrSequential` (Robot 72's non-alternating Y+R-Y+B-Y), `YCbCrLinePaired` (MP/PD's two-luma-lines-per-chroma-pair, added `RowsPerTransmissionLine` to `IScanlineEncoder`/`Decoder` to support it). A two-stage "extended VIS" mechanism (escape byte 0x23 + a second raw byte, see `VisHeader`) was added for the MR/ML/MP families.
  - **In progress, stopped mid-mode**: MN73/110/140 and MC110/140/180 (both `Main.cpp`'s `LineMN`/`LineMC`, already read). Both use a "narrow" frequency range (`NARROW_SYNC`=1900Hz, `NARROW_LOW`=2044Hz, `NARROW_HIGH`=2300Hz, via `ColorToFreqNarrow` in source) instead of the normal 1500–2300Hz. MN is line-paired (same Y-RY-BY-Y2 shape as MP, just narrow-range); MC is plain sequential R/G/B (fits `RgbSequential` as-is). **Found but not yet fixed**: `YCbCrLinePairedScanlineEncoder`/`Decoder`, `YCbCrSequentialScanlineEncoder`/`Decoder`, and `RobotScanlineEncoder`/`Decoder` all hardcode the 1500/2300 frequency range literally instead of reading `mode.LuminanceMinHz`/`LuminanceMaxHz` — `RgbSequentialScanlineEncoder`/`Decoder` already does this correctly (existing code, not a new fix). Fix that hardcoding first (affects MN, and is a real inconsistency regardless), then MC should just work by setting `LuminanceMinHz=2044`/`LuminanceMaxHz=2300` in the mode definition — no MC-specific code needed. **Not yet found**: MN73/110/140 and MC110/140/180's VIS codes (searched `sstv.cpp`'s VIS switch, not present in the sections already read — the switch may have more cases further down, or these might also use the extended-VIS mechanism; check both before assuming).
  - Remaining after MN/MC, each still needing its own from-source tracing: R24, RM8/RM12, and the SC2-180/120/60 family (`Main.cpp`'s `LineSC2180`, not yet read — likely Scottie-shaped given `SetSampFreq`'s constants but confirm from the actual TX function, not by resemblance, per the no-assumptions rule).
