...
- VFO card's Step pill — user decision 2026-08-26: "maybe one day," surfaced during Tier 4 (CAT-layer) scoping. No legacy precedent found at all (grepped `yoniq-old/YONIQ-main/cradio.h` directly — legacy's own CAT abstraction layer has zero mentions of a step concept), unlike its BW/Split/RIT siblings on the same card, all of which have a real (if unbound) Hamlib concept to eventually wire. Almost certainly meant a local UI frequency-nudge increment, not anything read from or set on the rig via CAT — there was nothing to scope as CAT-layer work at all, unlike the others. **Removed from the UI 2026-08-26** — pill and the `RadioStatus.StepValue` locale key deleted; RIT's own pill (same `StackPanel`, still a real, if unbuilt, CAT concept) is untouched.
