...
| Open in Log / Export frame / Re-decode buttons | STUB | `MainWindow.axaml:1099-1101` (renumbered, now further down after the new Note/Flagged rows) | No `Command` on any of the three. "Open in Log" specifically is NOT blocked on a missing primitive — `IReceiveHistoryStore.SetLinkedQsoIdAsync` is real (see "Discovered 2026-08-11" note below) but still needs a QSO picker/auto-create UI decision, not wired this pass; the other two still are ("Export frame" needs a file-copy+picker flow, small but unbuilt; "Re-decode" is likely permanently infeasible — only the final image is retained per entry, no raw audio is kept to re-run the decoder against). |
...
outright (they no longer exist as controls at all) rather than reclassifying them; the +2 later on
2026-08-11 being the two brand-new Note/Flagged controls added to the Gallery Selected-frame panel.
