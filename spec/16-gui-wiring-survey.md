...
### Decode tab — Auto-Sync/Auto-Slant wired 2026-08-11, Auto-stop/Auto-restart + Sense level + Demod type wired 2026-08-12

RX BPF, RX buffer, Auto-start remain `IsEnabled="False"` (`OptionsWindowView.axaml:341-346,358-372`).
RX BPF DOES have real legacy Options-dialog precedent (`Option.dfm`'s `RGRxBPF`) but needs its own,
larger DSP-scoping pass before wiring (a `CalcBPF`-equivalent FIR filter-preset port) — not touched
in this batch, separate future plan. RX buffer has **no** identified legacy Options-dialog precedent
at all (checked directly, not assumed) — likely needs correcting/repurposing like Auto-stop/
Auto-restart's label fix, not a straight wiring target. Auto-start has real legacy behavior (`SBAuto`
toolbar button) but lives on the legacy toolbar, not in `Option.dfm` — a placement question, not
investigated further this pass.
Demod type (`RGDemType`, `Option.dfm`) was the one remaining "needs its own larger DSP-scoping pass"
item from the previous version of this note — **now done**: the demod-type runtime-dispatch subsystem
(4 phases, 2026-08-12) ported the missing PLL narrow-mode support, wired real runtime dispatch
between the 3 already-ported demodulator classes into `AnalogFmSstvDecoder`, threaded the setting
through the settings layer, and un-stubbed this control — see that subsystem's own plan/commits for
...
cheap to wire — confirmed true for six of nine now (Auto-stop/Auto-restart needed a label fix first,
not just wiring; Sense level needed a genuine DSP-table port; Demod type needed a whole separate
4-phase subsystem, not just wiring), not for the remaining three controls (RX BPF needs its own
larger DSP-scoping pass, RX buffer has no identified legacy precedent, Auto-start is toolbar-only in
legacy).
