...
### Decode tab — Auto-Sync/Auto-Slant wired 2026-08-11, Auto-stop/Auto-restart wired 2026-08-12

Sense level, RX BPF, Demod type, RX buffer, Auto-start remain `IsEnabled="False"`
(`OptionsWindowView.axaml:318-371`) — no backing settings surface at all exists for any of them
(`AnalogFmSstvDecoder` hardcodes them internally).
...
cheap to wire — confirmed true for all four now (Auto-stop/Auto-restart needed a label fix first, not
just wiring), not for the other four controls (no backing settings surface at all).
