...
**Menu bar** (`MainWindow.axaml:67-97`) — unchanged since 2026-08-09 (line numbers only shifted). Only `File > Open image` (`TxControls.SelectImageCommand`) and `File > Exit` (`ExitCommand`) plus the `Options...` item (`OpenOptionsCommand`, `:97`) are REAL. Every other menu item across Configurations/Rig & PTT/Calibration/Tools is STUB (`IsEnabled="False"`, `Options.NotImplemented.Help` tooltip). `Help` menu has no submenu items at all (`MainWindow.axaml:96`). Callsign chip is REAL (`CallsignDisplay`, `:113`, loaded from `OptionsSettingsService`). **Not yet actioned, flagged 2026-08-11**: now that the Options window has a real "Radio / CAT" tab with working backend selection, the "Rig & PTT" top-menu (CAT Interface/PTT Method/Frequency Memories/Test PTT, all still STUB placeholders) reads as redundant, and "Configurations" may partly overlap too (e.g. `Configurations.AudioDevices` vs. the Options Audio tab) — a menu-trim pass, not yet scoped or started (see `PROJECT_BRIEF.md`).
