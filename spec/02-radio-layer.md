...
Define the hardware- and protocol-agnostic abstraction that everything above it (UI, logbook, SSTV session) programs against. Nothing outside `ScanlineStudio.Core.Radio` may know whether a rig is reached over a serial CAT cable, TCP, or rigctld — that detail lives entirely in [[03-cat-layer]] and [[04-rigctld]].
...
- [x] `IRadioProtocolFactory`-based backend resolution (exactly-one-match, typed error on zero/ambiguous
      ...
      `NoneRadioProtocolFactory`/`RigctldProtocolFactory`/`HamlibProtocolFactory`/`FlrigProtocolFactory`/
      `OmniRigProtocolFactory` are the five concrete implementations so far.
