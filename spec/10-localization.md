...
- [ ] `en.json` populated from `.dfm` caption/hint extraction and `ja.json` populated from `sys.m_MsgEng`-branch source mining (CP932-decoded) for every view as it's ported (tracked per-dialog in [[14-roadmap]]) — not from the legacy `.ini` files, which carry no string table. Only `en.json` exists so far — it has grown well past Phase 3's initial scope (now ~800 keys, used by `loc:Translate` across `MainWindow` and 7 other ported views: Options, TX image editor, RX image pane, QSO link window, About, radio header, TX controls) — still hand-authored, not `.dfm`-mined, since none of these views has a matching legacy dialog to mine from. `ja.json` and the `.dfm`-mining workflow are still open.
- [x] CI check for orphaned/missing locale keys and un-localized literal strings in `.axaml` files.
- [ ] Manual verification: switching language via the Options window's language selector (`OptionsWindowViewModel.SelectedCulture`/`AvailableCultures` — there is no separate `LanguageSettingsDialog`) updates all currently-open windows live, no restart.
