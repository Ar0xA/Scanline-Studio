...
- [ ] `en.json` populated from `.dfm` caption/hint extraction and `ja.json` populated from `sys.m_MsgEng`-branch source mining (CP932-decoded) for every view as it's ported (tracked per-dialog in [[14-roadmap]]) — not from the legacy `.ini` files, which carry no string table. Only `en.json` exists so far — it has grown well past Phase 3's initial scope (~890 keys as of 2026-08-30, used by `loc:Translate` across 19 views, essentially the whole `ScanlineStudio.UI` view set) — still hand-authored, not `.dfm`-mined, since none of these views has a matching legacy dialog to mine from. `ja.json` and the `.dfm`-mining workflow are still open.
