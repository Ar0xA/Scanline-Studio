...
Samples are always `float` in `[-1.0, 1.0]`, mono, at a DSP-chosen sample rate. **Default is 44100 Hz, not legacy's 11025 Hz** — corrected from an earlier revision of this section during piece Audio 2's contract review, when a chunk of the SSTV mode table still failed the standard round-trip tolerance at 11025Hz (a pixel-readout/demodulator-settling gap, tracked separately, not this engine's concern) and the existing `ScanlineStudio.Core.Sstv.Tests` suite ran at 44100Hz as a stated "pragmatic Phase 1 accommodation" for exactly that reason. **Update:** that specific 11025Hz gap has since been fixed for every previously-failing mode (`SstvModeRegistry`'s `CSSTVDEM` sync-search/AFC port — `AfcTracker`/`SyncIntervalTracker`/`SyncAnchorCorrector` — plus a per-line-cursor-rounding fix; see `SstvRoundTripTests.ModesPreviouslyFailingAt11025Hz`, all of which now measure well under tolerance at 11025Hz). The main round-trip suite still defaults to 44100Hz for the full mode table as its locked-in regression configuration, not because 11025Hz remains broken — 11025Hz stays available (any rate up to 48000Hz is) and is legacy-authentic. Format conversion from whatever the device natively supports happens inside the engine, never in `ScanlineStudio.Core.Sstv`.
