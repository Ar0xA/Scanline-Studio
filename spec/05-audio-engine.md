...
- [x] Native `miniaudio` binaries built from a pinned upstream tag (`0.11.25`) via a documented shim (`native/yoniq_audio.c`/`.h`) — license recorded in [LICENSES.md](../LICENSES.md). Per-RID packaging/CI matrix for all three OSes is still Linux-only (MSBuild target in each consuming project) — cross-platform native build automation is a separate, still-open item (see [[14-roadmap]]).
