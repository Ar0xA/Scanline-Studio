...
1. Copy `index.html` to a new `assets/help/<languageCode>/index.html`, and translate its visible
...
The Host project's build already copies any `assets/help/<languageCode>/index.html` alongside the
...
```sh
node assets/help/check-help.mjs
```
...
`assets/help/<languageCode>/index.html` it finds, and fails if a translated guide's topic-ID set
