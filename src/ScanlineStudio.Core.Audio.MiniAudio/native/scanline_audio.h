#ifndef SCANLINE_AUDIO_H
#define SCANLINE_AUDIO_H
...
#if defined(_WIN32)
#define SCANLINE_AUDIO_API __declspec(dllexport)
#else
#define SCANLINE_AUDIO_API
#endif

#define SCANLINE_AUDIO_ID_SIZE 256
#define SCANLINE_AUDIO_NAME_SIZE 256
#define SCANLINE_AUDIO_BACKEND_NAME_SIZE 32

typedef struct
{
    char id[SCANLINE_AUDIO_ID_SIZE];     /* backend-native device id, as a UTF-8/ASCII string */
    char name[SCANLINE_AUDIO_NAME_SIZE]; /* UTF-8 device name */
    int is_default;                   /* nonzero if this is the backend's default device */
} scanline_audio_device_info;
 ...
 * resolved backend's name into backend_name_out (must be >= SCANLINE_AUDIO_BACKEND_NAME_SIZE bytes).
 * Returns 0 on success, nonzero on failure. Must be called exactly once before any other function
 * here; not safe to call again without scanline_audio_context_uninit first. */
SCANLINE_AUDIO_API int scanline_audio_context_init(char *backend_name_out);

SCANLINE_AUDIO_API void scanline_audio_context_uninit(void);
...
SCANLINE_AUDIO_API int scanline_audio_enumerate_devices(int is_capture, scanline_audio_device_info *out_devices, int max_count);
...
} scanline_audio_native_format;

/* Probes the named device's native format capabilities -- unlike scanline_audio_enumerate_devices,
...
SCANLINE_AUDIO_API int scanline_audio_get_native_formats(const char *device_id, int is_capture, scanline_audio_native_format *out_formats, int max_count);

/* Spike/gate test only (not the production capture path -- see piece Audio 5 for that): opens
 * the named capture device (by the id string scanline_audio_enumerate_devices returned), captures
...
SCANLINE_AUDIO_API int scanline_audio_spike_capture_test(const char *device_id, int duration_ms, float *peak_out);
...
typedef struct scanline_audio_ring scanline_audio_ring;
...
SCANLINE_AUDIO_API scanline_audio_ring *scanline_audio_ring_create(int capacity_frames, int channels);

SCANLINE_AUDIO_API void scanline_audio_ring_destroy(scanline_audio_ring *ring);
...
SCANLINE_AUDIO_API int scanline_audio_ring_write(scanline_audio_ring *ring, const float *data, int frame_count);
...
SCANLINE_AUDIO_API int scanline_audio_ring_read(scanline_audio_ring *ring, float *out_data, int frame_count);

/* Returns how many frames are currently buffered in the ring, available to be read without
 * blocking. Returns -1 on error. */
SCANLINE_AUDIO_API int scanline_audio_ring_available_read(scanline_audio_ring *ring);
 ...
 * writes captured frames into an internal scanline_audio_ring; a managed drain thread/loop reads
 * from it via scanline_audio_capture_session_read. Requests mono f32 at the caller's chosen sample
...
typedef struct scanline_audio_capture_session scanline_audio_capture_session;

/* Shared by both scanline_audio_capture_session_open and scanline_audio_playback_session_open --
 ...
 * (scanline_audio_ring) and everything on the managed side of the boundary
...
typedef struct scanline_audio_open_options
{
    int sample_rate;
 ...
 * failure. */
SCANLINE_AUDIO_API scanline_audio_capture_session *scanline_audio_capture_session_open(const char *device_id, const scanline_audio_open_options *options);

/* Stops and destroys the session. Safe to call on a session that failed to fully start (i.e. a
 * partially-initialized state scanline_audio_capture_session_open itself cleans up on its own error
 * paths -- this is for a session that DID open successfully and is now done with). */
SCANLINE_AUDIO_API void scanline_audio_capture_session_close(scanline_audio_capture_session *session);
...
SCANLINE_AUDIO_API int scanline_audio_capture_session_read(scanline_audio_capture_session *session, float *out_data, int frame_count);
...
SCANLINE_AUDIO_API int scanline_audio_capture_session_check_and_clear_stopped(scanline_audio_capture_session *session);
 ...
 * overrun policy above). Mirrors scanline_audio_playback_session_underrun_count's own shape and
...
SCANLINE_AUDIO_API int scanline_audio_capture_session_overrun_count(scanline_audio_capture_session *session);

/*
 * Piece Audio 6: the real playback path -- the mirror image of piece Audio 5's capture session.
 * Managed code writes samples into an internal ring via scanline_audio_playback_session_write; the
...
typedef struct scanline_audio_playback_session scanline_audio_playback_session;
 ...
 * scanline_audio_open_options' own doc comment and scanline_audio_capture_session_open's identical
 * shape. Returns NULL on failure. */
SCANLINE_AUDIO_API scanline_audio_playback_session *scanline_audio_playback_session_open(const char *device_id, const scanline_audio_open_options *options);

SCANLINE_AUDIO_API void scanline_audio_playback_session_close(scanline_audio_playback_session *session);
...
SCANLINE_AUDIO_API int scanline_audio_playback_session_write(scanline_audio_playback_session *session, const float *data, int frame_count);
...
SCANLINE_AUDIO_API int scanline_audio_playback_session_pending_frames(scanline_audio_playback_session *session);
 ...
 * (scanline_audio.c's own underrun_count field comment) increments this exactly ONCE PER CALLBACK
...
SCANLINE_AUDIO_API int scanline_audio_playback_session_underrun_count(scanline_audio_playback_session *session);

SCANLINE_AUDIO_API int scanline_audio_playback_session_check_and_clear_stopped(scanline_audio_playback_session *session);
...
SCANLINE_AUDIO_API int scanline_audio_resample_f32(const float *input, int input_frame_count, int sample_rate_in,
 ...
 * session open on it -- there is no scanline_audio_capture_session/scanline_audio_playback_session
 * parameter here on purpose. Read-only by design: this shim never exposes a mute SETTER -- see
 * scanline_audio_get_device_mute's own doc comment for why. Reaches into each backend's real
...
SCANLINE_AUDIO_API int scanline_audio_get_device_mute(const char *device_id, int is_capture, int *is_muted_out);
