...
/// <see cref="Online"/> is the only status this backend proceeds past; every other value throws an
/// <c>IOException</c> (T1-10, production_audit.md -- a transport-level failure, not a command-level
/// one) carrying OmniRig's own <c>StatusStr</c> text -- see <c>OmniRigRadioProtocol.PollAsync</c>.</summary>
