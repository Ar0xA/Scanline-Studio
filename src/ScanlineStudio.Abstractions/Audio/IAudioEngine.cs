...
/// native `miniaudio` shim (see `ScanlineStudio.Core.Audio.MiniAudio`'s own doc comments), writing into a
...
/// (it hands out caller-owned memory), and the real `ScanlineStudio.Core.Audio.MiniAudio` implementation's
